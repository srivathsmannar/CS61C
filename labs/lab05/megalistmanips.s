...
main:
    ...
    # issue the map call
    add a0, s0, zero      # load the address of the first node into a0
...
map:
    ...
    add s0, a0, zero  ###    # save address of this node in s0
    add s1, a1, zero      # save address of function in s1
    add t0, zero, zero      # t0 is a counter
...
mapLoop:
    ...
    jalr s1
    lw t0, 0(sp)
    lw t1, 4(sp)             # call the function on that value.
    addi sp, sp, 8
